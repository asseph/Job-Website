...
export const AllJobListings: React.FC<JobListingProps> = ({ jobs }) => {
  ...
  return (
    <div className="overflow-y-auto grid grid-cols-1 lg:grid-cols-3 gap-5 text-center bg-accent-focus rounded-2xl p-4">
      {jobs?.map((job) => (
        <div className="text-left max-w-full flex flex-col m-2 bg-primary py-2 px-4 rounded-lg shadow-lg">
          <h1 className="text-2xl text-white font-bold">{job.title}</h1>

          <p className="text-white">
            Posted by: {job.posterpubkey.slice(0, 4)}...{" "}
            {job.posterpubkey.slice(-4)}
          </p>

          <div className=" mb-2 flex flex-col sm:flex-row sm:flex-wrap sm:space-x-3">
            <p className="text-white text-sm">Pay: {job.pay}</p>
            <p className="text-white text-sm">Timeline: {job.timeline}</p>
            <p className="text-white text-sm">Status: {job.status}</p>
          </div>
